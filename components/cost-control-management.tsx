...
import { AlertTriangle, Edit, Download, Upload } from "lucide-react"
...
interface CostControlData {
  itemId: number
  itemName: string
  dates: string[] // Unique dates for PICK UP entries within the filtered period
  totalPickedUpQuantity: number // Sum of all PICK UP quantities for this item within the filtered period
...
export function CostControlManagement({ user, logBookEntries }: { user: User; logBookEntries: LogEntry[] }) {
  ...
  const currentMonth = format(new Date(), "MM") // e.g., "07" for July
  const currentYear = format(new Date(), "yyyy") // e.g., "2025"
  ...
  const years = Array.from({ length: 5 }, (_, i) => (new Date().getFullYear() - i).toString()) // Current year and 4 previous years
  ...
  useEffect(() => {
    ...
    const aggregatedData: {
      [itemId: number]: { dates: Set<string>; totalOutQuantity: number }
    } = {}

    filteredLogEntries.forEach((entry) => {
      if (entry.outQuantity > 0) {
        // Only consider "PICK UP" quantities
        if (!aggregatedData[entry.itemId]) {
          aggregatedData[entry.itemId] = { dates: new Set(), totalOutQuantity: 0 }
        }
    ...
    itemDefinitions.forEach((itemDef) => {
      ...
      if (aggregated) {
        newCostControlData.push({
          ...
          dates: Array.from(aggregated.dates).sort(), // Sort dates for consistent display
          totalPickedUpQuantity: aggregated.totalOutQuantity,
          price: itemDef.price,
          totalCost: aggregated.totalOutQuantity * itemDef.price,
        })
      } else {
        // Include items that have no PICK UP entries but are in the default list
        newCostControlData.push({
          itemId: itemDef.id,
          itemName: itemDef.name,
          dates: [],
  ...
  }, [logBookEntries, itemDefinitions, selectedMonth, selectedYear]) // Dependencies: re-run when log entries, item definitions, or period change
  ...
  const handleSaveEdit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Update the price in the itemDefinitions state
  ...
  const generateCsvData = () => {
    const headers = ["No", "ITEM", "DATE", "QTY", "PRICE (Rp)", "TOTAL COST (Rp)"]
    const csvRows = []
    ...
    costControlData.forEach((data, index) => {
      const row = [
        index + 1,
        `"${data.itemName}"`, // Wrap in quotes to handle commas in names
        `"${data.dates.join(", ")}"`, // Join dates with comma and wrap in quotes
  ...
  const generateXlsxData = () => {
    const dataForXlsx = costControlData.map((data, index) => ({
      ...
      QTY: data.totalPickedUpQuantity > 0 ? data.totalPickedUpQuantity : "-",
      "PRICE (Rp)": data.price,
    ...
    if (sendEmail) {
      ...
      try {
        const response = await fetch("/api/send-report", {
          ...
          body: JSON.stringify({
            recipientEmail: user.email, // Menggunakan email pengguna yang login sebagai penerima
    ...
    reader.onload = (e) => {
      try {
        ...
        if (file.name.endsWith(".csv")) {
          ...
          // Use header: 1 to get the first row as headers, then map to desired keys
          ...
          importedItems = json
            ...
            .filter((row) => typeof row.ITEM === "string" && !isNaN(row["PRICE (Rp)"])) // Filter out invalid rows
        } else {
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm sm:text-base text-gray-600">
          Kelola dan pantau biaya operasional CM Coin Laundry berdasarkan data Loog Book.
        </p>
      </div>
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardHeader>
          <CardTitle className="flex items-center justify-between text-blue-800">
            ...
            <div className="text-xl font-bold">Total: Rp {totalAllCosts.toLocaleString("id-ID")}</div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-blue-700">
            Data biaya dihitung berdasarkan jumlah PICK UP dari Loog Book dalam periode{" "}
            <span className="font-semibold">
              {months.find((m) => m.value === selectedMonth)?.label} {selectedYear}
            </span>
            , dikalikan dengan harga per item.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tabel Cost Control</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      {/* All Items Price Reference (for editing prices) */}
      <Card>
        <CardHeader>
